...
def _training_inventory_rows(items: list[dict[str, str]]) -> list[list[str]]:
    return [[item["name"], item["code_example"], item["nature_precise"], item["initial_value"], item["explored_values"], item["retained_value"], item["influence"], item["location"]] for item in items]
